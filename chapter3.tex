\chapter{Hierarchical Intervention}
...
\section{K-means clustering/Voronoi partitioning}
...
\section{Hierarchical Greedy Clustering}
...
Greedy Hierarchical Clustering turned out to have a major flaw. It selects seed points randomly. This means that in each level, the clusters can be highly imbalanced. We compared this approach to K means(Voronoi partitioning)\citep{hartigan1979algorithm,aurenhammer2000voronoi}, R Tree\citep{guttman1984r} and R* Tree\citep{beckmann1990r}. 
...
\begin{algorithm}
\caption{Algorithm for K means}\label{alg:kmeans}
\begin{algorithmic}[1]
...
\Procedure{ClusterOnce}{$points, k$}
    ...
    \State $centroids \gets random\_point \cup random\_point$
    \State $i \gets i+1$
    \EndFor
